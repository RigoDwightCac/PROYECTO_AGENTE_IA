CREATE DATABASE  IF NOT EXISTS `human_resources` /*!40100 DEFAULT CHARACTER SET utf8mb3 */ /*!80016 DEFAULT ENCRYPTION='N' */;
USE `human_resources`;
...
/*!50001 VIEW `emp_details_view` AS select `e`.`employee_id` AS `employee_id`,`e`.`job_id` AS `job_id`,`e`.`manager_id` AS `manager_id`,`e`.`department_id` AS `department_id`,`d`.`location_id` AS `location_id`,`l`.`country_id` AS `country_id`,`e`.`first_name` AS `first_name`,`e`.`last_name` AS `last_name`,`e`.`email` AS `email`,`e`.`phone_number` AS `phone_number`,`e`.`hire_date` AS `hire_date`,`e`.`salary` AS `salary`,`e`.`commission_pct` AS `commission_pct`,`d`.`department_name` AS `department_name`,`j`.`job_title` AS `job_title`,`l`.`city` AS `city`,`l`.`state_province` AS `state_province`,`c`.`country_name` AS `country_name`,`r`.`region_name` AS `region_name` from (((((`employees` `e` join `departments` `d` on((`e`.`department_id` = `d`.`department_id`))) join `jobs` `j` on((`e`.`job_id` = `j`.`job_id`))) join `locations` `l` on((`d`.`location_id` = `l`.`location_id`))) join `countries` `c` on((`l`.`country_id` = `c`.`country_id`))) join `regions` `r` on((`c`.`region_id` = `r`.`region_id`))) */;
...
-- Dump completed on 2025-11-05  9:16:17
